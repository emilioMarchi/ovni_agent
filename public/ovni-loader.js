...
(function(){
  ...
  window.OvniWidget = {
    init: function(config) {
      if (document.getElementById('ovniWidgetHost')) return;
      var resolvedApiUrl = resolveApiUrl(config);
      var logoUrl = resolvedApiUrl.replace(/\/$/, '') + '/logo.png';

      // Crear host element y Shadow DOM para aislamiento total
      ...
      class OvniChatWidget {
        ...
        init() {
          const toggle = this.$('ovniToggle');
          const send = this.$('ovniSend');
          const input = this.$('ovniInput');
          ...
          const audioBtn = this.$q('.audio-trigger');
          ...
          const messagesEl = this.$('ovniMessages');
          if (messagesEl) {
        ...
        async loadAgentInfo() {
          try {
            ...
            this.$('agentName').textContent = agentName;
            ...
            this.$('agentName').textContent = this.agentName || 'Asistente';
        ...
        showEmptyHint(name) {
          const container = this.$('ovniMessages');
        ...
        addMessage(content, type) {
          ...
          let html = '';
          if (type === 'assistant') {
            html += `<img src="${this.logoUrl}" class="avatar">`;
          }
        ...
        showTyping() {
          ...
          wrapper.innerHTML = `
            <img src="${this.logoUrl}" class="avatar">
      ...
      // Inicializar el widget con la config pasada y el shadow root
      window.ovniChat = new OvniChatWidget(config, shadow);
    }
  };
})();
