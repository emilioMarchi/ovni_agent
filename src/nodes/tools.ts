 ...
 */
function injectAllowedDocIds(message: any, state: AgentStateType): any {
...
export async function toolNodeWithLogs(state: AgentStateType) {
  ...
  if (patchedMessage.tool_calls && Array.isArray(patchedMessage.tool_calls)) {
    ...
    if (allowedDocIds.length === 0) blockedTools.add("knowledge_retriever");

    const filtered = patchedMessage.tool_calls.filter((tc: any) => {
    ...
    if (filtered.length === 0 && patchedMessage.tool_calls.length > 0) {
      ...
      const blockedResponses = patchedMessage.tool_calls.map((tc: any) => 
        new ToolMessage({
    ...
    if (filtered.length < patchedMessage.tool_calls.length) {
      ...
      const { ToolMessage } = await import("@langchain/core/messages");
      const blockedCalls = patchedMessage.tool_calls.filter((tc: any) => blockedTools.has(tc.name));
      const blockedResponses = blockedCalls.map((tc: any) =>
        new ToolMessage({
          content: "Esta herramienta no está disponible para este agente.",
          tool_call_id: tc.id,
          name: tc.name,
