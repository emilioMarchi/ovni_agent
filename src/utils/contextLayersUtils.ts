...
import { getRecentMessages } from "./historyUtils.js";
 ...
 */
export async function getOlderMessages(threadId: string, skip: number = 15, limit: number = 15) {
 ...
 * @returns {Promise<Array<{role: string, content: string, timestamp: string}>>}
 */
export async function semanticSearchHistory(userId: string, query: string, limit: number = 10) {
  ...
  snapshot.forEach(doc => {
    const data = doc.data();
    const messages = data.messages || [];
    results = results.concat(messages.filter((m: any) =>
