...
function hourToWords(hour24: number): string {
  const hour12 = hour24 % 12 || 12;
  if (hour12 === 1) return 'una';
  return numberToWords(hour12);
}

function formatTimeForSpeech(hour: number, minutes: number): string {
...
function normalizeColloquialTimes(text: string): string {
  return text
...
function normalizeDigitalTimes(text: string): string {
