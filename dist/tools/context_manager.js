...
export function getSessionData(threadId) {
...
- "get_summary": resume todo lo que sabés del usuario y la conversación`,
    ...
    func: async ({ action, threadId, userData, businessData, field }) => {
        ...
        if (action === "get") {
            ...
            if (field === "user")
                return JSON.stringify(ctx.userInfo);
            if (field === "business")
                return JSON.stringify(ctx.businessInfo);
...
export function clearSessionContext(threadId) {
