...
const oauth2Client = new google.auth.OAuth2(getRequiredEnv("GOOGLE_CLIENT_ID"), getRequiredEnv("GOOGLE_CLIENT_SECRET"), getRequiredEnv("GOOGLE_REDIRECT_URI"));
const SCOPES = ["https://www.googleapis.com/auth/calendar"];
export async function getCalendarClient(clientId) {
...
export async function createCalendarEvent(clientId, meetingData) {
...
export async function updateCalendarEvent(clientId, eventId, meetingData) {
    const calendar = await getCalendarClient(clientId);
    const updates = {};
...
export async function deleteCalendarEvent(clientId, eventId) {
    const calendar = await getCalendarClient(clientId);
...
export async function handleOAuthCallback(code, clientId) {
...
export async function isCalendarConnected(clientId) {
    try {
        const db = admin.firestore();
