...
const agentClientCache = new Map();
const adminDomainsCache = new Map();
...
function getWidgetTokenFromRequest(req) {
...
function isWidgetTokenRoute(req) {
    return req.path === "/widget-token";
}
function shouldRequireWidgetToken(req) {
...
function verifyWidgetToken(token) {
    ...
    try {
        ...
        if (payload.sub !== "widget")
            return null;
        if (!payload.clientId || !payload.originHost || !payload.expiresAt)
            return null;
...
function matchesAllowedDomain(host, allowedDomain) {
...
function extractClientIdCandidate(req) {
    const body = req.body && typeof req.body === "object" ? req.body : {};
...
function extractAgentIdCandidate(req) {
    const body = req.body && typeof req.body === "object" ? req.body : {};
...
export function getRequestOriginHost(req) {
...
export function isWidgetTokenProtectionEnabled() {
...
export async function resolveClientId(agentId, providedClientId) {
    if (providedClientId) {
...
export async function getAllowedDomainsForClient(clientId) {
    if (!clientId)
        return [];
...
export function invalidateAllowedDomainsCache(clientId) {
    if (!clientId)
        return;
    adminDomainsCache.delete(clientId);
}
export async function validateWidgetAccess(req) {
...
export function widgetAccessGuard(req, res, next) {
...
function buildRateLimitKey(req, prefix) {
...
function sweepExpiredRateLimits() {
    ...
    if (rateLimitSweepCounter % 250 !== 0)
        return;
    const now = Date.now();
...
//# sourceMappingURL=widgetSecurity.js.map