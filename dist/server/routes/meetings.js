...
router.get("/google-calendar/callback", async (req, res) => {
    ...
    try {
        await handleOAuthCallback(code, clientId);
...
router.all("/:meetingId/confirm", async (req, res) => {
    try {
        ...
        if (calendarConnected && !calendarEventId) {
            ...
            if (!calendarConnected)
                console.warn(`⚠️ [CALENDAR] OMITIDO: El calendario NO está conectado para el cliente ${clientId}.`);
            if (calendarEventId)
                console.warn(`⚠️ [CALENDAR] OMITIDO: La reunión ya tiene un evento asociado (ID: ${calendarEventId}).`);
        }
...
router.all("/:meetingId/reject", async (req, res) => {
    try {
        const { meetingId } = req.params;
        const db = admin.firestore();
        const meetingRef = db.collection("meetings").doc(meetingId);
