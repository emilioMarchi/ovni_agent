...
router.put("/:id", async (req, res) => {
    try {
        ...
        if (businessContext !== undefined)
            updates.businessContext = businessContext;
        if (allowedDomains !== undefined)
            updates.allowedDomains = normalizeAllowedDomains(allowedDomains);
        ...
        try {
            if (process.env.PINECONE_API_KEY) {
                ...
                if (stats.namespaces) {
                    ...
                    const namespacesToDelete = Object.keys(stats.namespaces).filter(ns => ns === `client_${adminId}` ||
